...
export function addObserveTarget<T>(target: HTMLElement | Window | null, affix?: T): void {
  ...
  TRIGGER_EVENTS.forEach((eventName) => {
    instance!.eventHandlers[eventName] = addEventListener(target, eventName, () => {
...
export function removeObserveTarget<T>(affix: T): void {
  ...
  const observerInstance = observerInstances.find((observerInstancesItem) => {
    // 监听事件数组中是否有当前节点
    const hasAffix = observerInstancesItem.affixList.some((item) => item === affix);
  ...
  if (observerInstance && observerInstance.affixList.length === 0) {
    observerInstances = observerInstances.filter((item) => item !== observerInstance);
